...
try {
  ...
  const disableByEnv = process.env.LYRICDISPLAY_DISABLE_KEYTAR === '1' || process.env.DISABLE_KEYTAR === '1';
  if (isWsl || disableByEnv) {
    keytar = null;
    console.log('Keytar disabled for WSL/Linux without secret service');
  } else {
...
class SimpleSecretManager {
  ...
  async _readFromKeytar() {
    ...
    if (process.env.LYRICDISPLAY_DISABLE_KEYTAR === '1' || process.env.DISABLE_KEYTAR === '1') return null;
    try {
      const timeout = new Promise((resolve) => setTimeout(() => resolve(null), 2000));
      const read = keytar.getPassword(SERVICE_NAME, ACCOUNT_NAME).catch(() => null);
  ...
  async _writeToKeytar(dataStr) {
    ...
    if (process.env.LYRICDISPLAY_DISABLE_KEYTAR === '1' || process.env.DISABLE_KEYTAR === '1') return false;
    try {
      const timeout = new Promise((resolve) => setTimeout(() => resolve(false), 2000));
      const write = keytar.setPassword(SERVICE_NAME, ACCOUNT_NAME, dataStr).then(() => true).catch(() => false);
