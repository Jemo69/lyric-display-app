...
const StageOutput = ({ outputKey = 'stage', displayName = 'Stage' }) => {
    ...
    useEffect(() => {
        ...
        const handleStyleUpdate = (data) => {
            ...
            if (data && data.output === outputKey && data.settings) {
                useLyricsStore.getState().updateOutputSettings(outputKey, data.settings);
            }
        };
    ...
    useEffect(() => {
        if (!maxLinesEnabled || !isVisible) return;
    ...
    useEffect(() => {
        ...
        const calculate = () => {
            const currentLineObj = currentLine >= 0 && currentLine < lyrics.length ? lyrics[currentLine] : null;
            const lineCount = lineText.split('\n').length;
            const lineScales = currentLineObj?.type === 'group' && lineCount === 2 ? [1, 0.8] : null;
            const result = calculateOptimalFontSize({
                ...
                maxLinesEnabled,
                containerWidth: autoScaleBounds.width,
                containerHeight: autoScaleBounds.height,
                lineScales,
            });
            ...
            setAdjustedFontSize((previousSize) => {
                if (previousSize !== null && safeAdjusted !== null && Math.abs(previousSize - safeAdjusted) < 2) {
                    return previousSize;
                }
                return safeAdjusted;
            });
        ...
        if (document.fonts?.ready) {
            document.fonts.ready.then(() => {
                if (!cancelled) scheduleCalculate();
            });
        } else {
            scheduleCalculate();
        }
    ...
    const showMotionBg = isMotionBackgroundType && (isOutputOn || alwaysShowBackground || isStageCleared);
