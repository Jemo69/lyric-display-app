...
export default function LyricVisualFrame({
  ...
  const { lyricsFileName, bibleVersion } = useLyricsState();
  const safeSettings = settings || {};
  const rawDisplayLine = typeof line === 'string' ? line : (getLineOutputText(currentLine) || '');

  const bibleReferencePosition = safeSettings.bibleReferencePosition || 'bottom-center';
  const bibleReferenceSize = safeSettings.bibleReferenceSize ?? 28;
  const showBibleVersion = safeSettings.showBibleVersion !== false;
  const extractBibleVerseParts = (fullText, referenceText) => {
    if (!referenceText || typeof fullText !== 'string') return { body: fullText, reference: referenceText || '' };
    const trimmedRef = referenceText.trim();
    if (!trimmedRef) return { body: fullText, reference: '' };
    return { body: fullText, reference: trimmedRef };
  };
  const { body: displayLineBody, reference: bibleReferenceText } = extractBibleVerseParts(rawDisplayLine, lyricsFileName);
  const displayLine = displayLineBody;
  const bibleReferenceDisplay = showBibleVersion && bibleReferenceText ? formatBibleReference(bibleReferenceText, bibleVersion) : bibleReferenceText;
  const getBibleReferenceOverlayStyle = () => {
    const base = {
      position: 'absolute',
      fontSize: `${bibleReferenceSize}px`,
      fontFamily: safeSettings.fontStyle || 'Bebas Neue',
      color: '#FFFFFF',
    ...
    switch (bibleReferencePosition) {
      ...
      case 'bottom-center':
      default: return { ...base, bottom: '1rem', left: '50%', transform: 'translateX(-50%)' };
    }
  };
  ...
  return (
    ...
    >
      ...
      {isVisible && bibleReferenceDisplay && (
        <div style={getBibleReferenceOverlayStyle()}>
